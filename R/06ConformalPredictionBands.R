...
full_calibration_data <- suppressMessages(read_csv("./data/full_data_test_balanced.csv"))
area_calibration_data <- suppressMessages(read_csv("./data/area_data_test_balanced.csv"))
spat_calibration_data <- suppressMessages(read_csv("./data/spatial_data_test_balanced.csv"))

full_test_data <- suppressMessages(read_csv("./data/full_data_calibration_balanced.csv"))
area_test_data <- suppressMessages(read_csv("./data/area_data_calibration_balanced.csv"))
spat_test_data <- suppressMessages(read_csv("./data/spatial_data_calibration_balanced.csv"))
...
get_conformal_scores <- function(data_model, calibration_data) {
  data_model |> 
    ...
    mutate(conformal_score = ifelse(calibration_data |> pull(Cancerous), .pred_FALSE, .pred_TRUE)) |> 
...
get_conformal_classes <- function(conformal_scores, test_preds, agreement, alpha) {
  ...
  test_preds |> 
    mutate(
      .pred_FALSE = .pred_FALSE > 1-target_quantile,
      .pred_TRUE = .pred_TRUE > 1-target_quantile,
      agreement = agreement
    )
}
